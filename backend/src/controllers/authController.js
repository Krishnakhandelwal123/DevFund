...
const login = async (req, res) => {
  ...
  try {
    ...
    // If the account was created via Google, they should log in via Google.
    // Prevent password-based login attempts against Google-only accounts.
    if (user.authMethod === 'google' || !user.password) {
      return res.status(400).json({ message: "This account was created with Google Sign-In. Please log in with Google." });
    }
...
const becomeCreator = async (req, res) => {
  try {
    ...
    // Create stats doc for this creator (if not already created)
